...
void usercontrol(void) {
  ...
  while (1) {
    ...
    if (loop_cnt % 3 == 0) {
      angle = imu.rotation();
      rotations = (RightDriveSmart.position(turns) + LeftDriveSmart.position(turns)) / 2;
      Controller.Screen.setCursor(1, 1);
      Controller.Screen.print("%+03.2f, %+03.2f, %+03.2f", dist_sensor.objectDistance(inches), angle, rotations);
    }
